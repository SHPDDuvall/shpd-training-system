...
const Dashboard: React.FC<DashboardProps> = ({ onViewChange }) => {
  ...
  const pendingApprovals = useMemo(() => {
    ...
    return allRequests.filter(r => {
      // Admins and Coordinators see everything pending
      if (user.role === 'administrator' || user.role === 'training_coordinator') {
        return r.status !== 'approved' && r.status !== 'denied' && r.status !== 'completed';
      }

      // Check if user is the assigned approver for the current step
      const isCurrentApprover = 
        (r.step1Id === user.id && !r.step1ApprovalDate) ||
        (r.step2Id === user.id && r.step1ApprovalDate && !r.step2ApprovalDate) ||
        (r.step3Id === user.id && r.step2ApprovalDate && !r.step3ApprovalDate) ||
        (r.step4Id === user.id && r.step3ApprovalDate && !r.step4ApprovalDate) ||
        (r.step5Id === user.id && r.step4ApprovalDate && !r.step5ApprovalDate);
      ...
      if (user.role === 'supervisor') {
        ...
        return requestingUser?.supervisorId === user.id && (r.status === 'submitted' || r.status === 'supervisor_review');
      }
